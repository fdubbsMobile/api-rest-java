...
/**
 * A resource pool implementation which scales the number of resources
 * based on pool usage.  As resource demand increases, the pool will
 * create new resources as needed.  As demand decreases, the pool will
 * free resources and scale itself down.  This resource pool is ideal
 * for managing resources that are cheap to create, but have a large
 * memory footprint.
 *
 * This implementation is thread-safe.
 *  
 * @author Evan Worley
 * @param <T> The type of resource this pool will manage
 */
public final class ScalingResourcePool<T> implements ResourcePool<T> {
    ...
    /**
     * @param factory The factory to create new resources with
     * @param scaleCheckInterval The time in milliseconds in between checks for scale
     * down opportunities.
     * @throws IllegalArgumentException If factory is null
     */
    public ScalingResourcePool(ResourceFactory<T> factory, int scaleCheckInterval)
    ...
    /**
     * Creates a ScalingResourcePool with a default of 1000ms between
     * scale down checks
     * @param factory The factory to create new resources with
     * @throws IllegalArgumentException If factory is null
     */
    public ScalingResourcePool(ResourceFactory<T> factory) throws IllegalArgumentException {
    ...
    /**
     * Manages the scaling down of the resource pool.  If this runner
     * observes an up-trend or static amount of available resources it will
    ...
    private class ScaleDownRunner implements Runnable {
        ...
        public void run() {
            while (enabled) {
                ...
                if (freeResources > 0 && freeResources >= lastCheckFreeResources) {
                    resources.pop();
