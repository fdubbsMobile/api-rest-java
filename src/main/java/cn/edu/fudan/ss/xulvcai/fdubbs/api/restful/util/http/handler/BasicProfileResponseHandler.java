...
public class BasicProfileResponseHandler implements
		ResponseHandler<BasicProfile> {

	private static Logger logger = LoggerFactory
			.getLogger(BasicProfileResponseHandler.class);
	...
	private BasicProfile doLoginAndGetBasicProfile()
			throws ClientProtocolException, IOException {
	...
	private BasicProfile constructBasicProfile(
			DomParsingHelper domParsingHelper, String xpathExpression) {

		String post = domParsingHelper.getAttributeTextValueOfNode("post",
				xpathExpression, 0);
		String login = domParsingHelper.getAttributeTextValueOfNode("login",
				xpathExpression, 0);
		String stay = domParsingHelper.getAttributeTextValueOfNode("stay",
				xpathExpression, 0);

		String year = domParsingHelper.getAttributeTextValueOfNode("year",
				xpathExpression, 0);
		String month = domParsingHelper.getAttributeTextValueOfNode("month",
				xpathExpression, 0);
		String day = domParsingHelper.getAttributeTextValueOfNode("day",
				xpathExpression, 0);

		String gender = domParsingHelper.getAttributeTextValueOfNode("gender",
				xpathExpression, 0);
		String lastLoginIp = domParsingHelper.getAttributeTextValueOfNode(
				"host", xpathExpression, 0);
		String lastLoginTime = domParsingHelper.getAttributeTextValueOfNode(
				"last", xpathExpression, 0).replace('T', ' ');
		String registerDate = domParsingHelper.getAttributeTextValueOfNode(
				"since", xpathExpression, 0).replace('T', ' ');
		...
		UserMetaData userMetaData = new UserMetaData().withNick(nick)
				.withGender(gender).withPostCount(postCount)
				.withLoginCount(loginCount).withLastLoginIp(lastLoginIp)
		...
		BasicProfile basicProfile = new BasicProfile()
				.withUserMetaData(userMetaData).withBirthDate(birthDate)
				.withOnlineTime(onlineTime).withRegisterDate(registerDate);
