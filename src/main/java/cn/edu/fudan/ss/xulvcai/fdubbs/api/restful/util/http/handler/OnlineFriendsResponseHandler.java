...
public class OnlineFriendsResponseHandler  implements ResponseHandler<List<Friend>> {

	private static Logger logger = LoggerFactory.getLogger(OnlineFriendsResponseHandler.class);
	...
	private List<Friend> doLoginAndGetOnlineFriends() throws ClientProtocolException, IOException {
	...
	public HttpGet getOnlineFriendsGetRequest() {
		URI uri = null;
		try {
			uri = new URIBuilder().setScheme("http").setHost(BBSHostConstant.getHostName()).setPath("/bbs/ovr").build();
		...
		if (uri == null) {
			return new HttpGet("http://"+BBSHostConstant.getHostName()+"/bbs/ovr");
		}
		else {
