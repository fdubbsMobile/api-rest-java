...
public class AllFriendsResponseHandler implements ResponseHandler<List<Friend>> {
	...
	private static Logger logger = LoggerFactory.getLogger(AllFriendsResponseHandler.class);
	...
	public List<Friend> handleResponse(HttpResponse response)
		...
		for(int index = 0; index < nodeCount; index++) {
			Friend friend = constructFriend(domParsingHelper, xpathOfFriend, index);
	...
	private List<Friend> doLoginAndGetAllFriends() throws ClientProtocolException, IOException {
	...
	public HttpGet getAllFriendsGetRequest() {
		URI uri = null;
		try {
			uri = new URIBuilder().setScheme("http").setHost(BBSHostConstant.getHostName()).setPath("/bbs/fall").build();
		...
		if (uri == null) {
			return new HttpGet("http://"+BBSHostConstant.getHostName()+"/bbs/fall");
		}
		else {
	...
	private Friend constructFriend(DomParsingHelper domParsingHelper, String xpathExpression, int index) {
