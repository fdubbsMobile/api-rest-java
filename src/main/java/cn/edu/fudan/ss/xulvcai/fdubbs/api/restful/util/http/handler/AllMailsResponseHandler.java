...
public class AllMailsResponseHandler implements
		ResponseHandler<MailSummaryInbox> {

	private static Logger logger = LoggerFactory
			.getLogger(AllMailsResponseHandler.class);
	...
	private MailSummaryInbox doLoginAndGetAllMails()
			throws ClientProtocolException, IOException {
	...
	private MailSummaryInbox constructInbox(DomParsingHelper domParsingHelper) {
		...
		MailSummaryInbox inbox = new MailSummaryInbox()
				.withStartMailNum(startCount).withTotalCount(totalCount)
				.withMailCount(mailCount).withMailSummaryList(mails);

		validateAndAdjustMailList(inbox, startNum);
		return inbox;
	}

	private MailSummary constructMail(DomParsingHelper domParsingHelper,
			String xpathExpression, int index, int startNum, int mailCount) {

		String isRead = domParsingHelper.getAttributeTextValueOfNode("r",
		...
		MailSummary mail = new MailSummary().withMailMetaData(metaData)
				.withIsNew("0".equals(isRead)).withMarkSign(markSign);
	...
	private void RemoveRedundantMails(List<MailSummary> mailSummaryList,
			int redundantNum) {
		for (int index = 0; index < redundantNum; index++) {
			...
			mailSummaryList.remove(mailSummaryList.size() - 1);// remove the
																// tail
		}
	}
