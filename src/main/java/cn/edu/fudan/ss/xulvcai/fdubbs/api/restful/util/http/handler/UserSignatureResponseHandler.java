...
public class UserSignatureResponseHandler implements
		ResponseHandler<UserSignature> {

	private static Logger logger = LoggerFactory
			.getLogger(UserSignatureResponseHandler.class);
	...
	public UserSignature handleResponse(HttpResponse response)
		...
		String signature = domParsingHelper
				.getTextValueOfSingleNode(xpathOfSignature);
	...
	public HttpGet getUserSignatureGetRequest() {
		...
		try {
			uri = new URIBuilder().setScheme("http")
					.setHost(BBSHostConstant.getHostName()).setPath("/bbs/sig")
					.build();
	...
	private UserSignature doLoginAndGetUserSignature()
			throws ClientProtocolException, IOException {
