...
public class AllSectionsResponseHandler implements ResponseHandler<List<SectionMetaData>> {

	private static Logger logger = LoggerFactory.getLogger(AllSectionsResponseHandler.class);
	...
	public HttpGet getAllSectionsGetRequest() {
		URI uri = null;
		try {
			uri = new URIBuilder().setScheme("http").setHost(BBSHostConstant.getHostName()).setPath("/m/bbs/sec").build();
		...
		if (uri == null) {
			return new HttpGet("http://"+BBSHostConstant.getHostName()+"/m/bbs/sec");
		}
		else {
