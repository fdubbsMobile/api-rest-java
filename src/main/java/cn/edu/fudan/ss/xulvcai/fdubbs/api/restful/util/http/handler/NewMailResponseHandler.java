...
public class NewMailResponseHandler implements ResponseHandler<List<MailSummary>> {

	private static Logger logger = LoggerFactory.getLogger(NewMailResponseHandler.class);
	...
	public HttpGet getNewMailGetRequest() {
		URI uri = null;
		try {
			uri = new URIBuilder().setScheme("http").setHost(BBSHostConstant.getHostName()).setPath("/bbs/newmail").build();
		...
		if (uri == null) {
			return new HttpGet("http://"+BBSHostConstant.getHostName()+"/bbs/newmail");
		}
		else {
	...
	private List<MailSummary> doLoginAndGetNewMail() throws ClientProtocolException, IOException {
