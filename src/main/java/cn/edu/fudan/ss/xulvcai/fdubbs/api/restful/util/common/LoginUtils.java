	...
	public static boolean isLoginNeeded(int statusCode, 
		...
		if(HttpStatus.SC_OK == statusCode) {
			if (httpContentType == HttpContentType.HTML_TYPE 
				...
				String errorMsg = HttpParsingHelper.getErrorMessageFromResponse(domParsingHelper);
	...
	public static HttpPost getLoginPostRequest(String user_id, String passwd) {
		...
		try {
			uri = new URIBuilder().setScheme("http")
					.setHost(BBSHostConstant.getHostName()).setPath("/bbs/login").build();
		...
		HttpPost httpPost;
		if (uri == null) {
			httpPost = new HttpPost("http://"+BBSHostConstant.getHostName()+"/bbs/login");
		}
		else {
