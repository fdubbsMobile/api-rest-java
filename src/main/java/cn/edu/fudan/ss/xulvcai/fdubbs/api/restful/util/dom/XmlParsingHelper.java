...
public class XmlParsingHelper implements DomParsingHelper {
	...
	private XmlParsingHelper(String xmlContent) throws Exception {
		String contentAsString = xmlContent.replaceAll("&amp;nbsp;", " ")
				.replaceAll("&#160;", " ");
		doc = DocumentHelper.parseText(contentAsString);
	}

	public static XmlParsingHelper parseText(String xmlContent)
			throws Exception {
	...
	public Content getContentValueofNode(String xpathExpression) {
		...
		for (int index = 0; index < nodeNum; index++) {
			...
			if (node.hasContent() && node.getNodeType() == Node.ELEMENT_NODE) {
				getContentValueofElementNode(content, stringBuilder,
						(Element) node);
	...
	@SuppressWarnings("unchecked")
	private void getContentValueofElementNode(Content content,
		...
		for (Node node : childNodes) {
			...
			if (node.getNodeType() == Node.TEXT_NODE) {
				...
				parseParagraphOnElementNode(content, stringBuilder,
						(Element) node);
			} else {
	...
	private String getAttributeValueOnLinkNode(Node node, String attributName) {
		...
		logger.debug("first_index : " + first_index + " , second_index : "
				+ second_index);
		String attributeValue = xmlString.substring(first_index + 1,
				second_index);
